# 本项目模拟日式西幻世界观中法师吟唱施法的过程。玩家在主线程的输入框中键入或念诵咒语（如"烈焰风暴"、"冰霜之雨"），Web Worker 中的轻量 AI 模型对咒语进行语义解析，提取元素类型（火/水/风/土）、法术形态（球/雨/墙/爆发）、威力等级等参数。主线程根据解析结果调用对应的 Three.js 粒子特效系统，并在场景中执行范围伤害判定。
...
玩家在 3D 场景中输入中文咒语（如「烈焰风暴」「冰霜之雨」「感电风暴」），系统在独立 **Web Worker** 中解析出元素、法术形态、威力与范围，主线程据此生成 **three.quarks** 粒子特效，并对场景中的训练假人进行球形范围伤害判定。
...
| 元素反应 | ✅ | 10 种双/三元素反应（见 `ReactionTable.ts`） |
| 粒子特效 | ✅ | six 单元素 JSON + 代码 Builder 兜底；复合反应代码预制 |
| 战斗演示 | ✅ | 3 个 Capsule 假人、距离衰减伤害、受击闪红 |
| UI | ✅ | Loading、符文实时高亮、施法栏、伤害飘字（GSAP） |
| VFX 导出脚本 | ✅ | `npm run vfx:export` 生成 `public/vfx/*.json` |

**说明**： **ONNX 轻量模型推理**、**Web Audio 音效**、**对象池** 等尚未接入；当前 Worker 使用**规则引擎**，不依赖外部模型文件。
...
1. **AI 解析升级**：训练/导出 `spell_parser.onnx`，在 Worker 中接入 ONNX Runtime Web，规则引擎作降级兜底。
2. **形态差异化特效**：按 `form`（雨/墙/束/风暴等）分支缩放与粒子编排，而不只调 `scale`。
3. **音效**：Web Audio API，按 `potency` / 元素反应播放吟唱与命中音。
4. **性能**：粒子实例对象池、Bloom 半分辨率、移动端 `devicePixelRatio` 策略。
5. **玩法扩展**：语音输入（Web Speech API）、法术书存档、联机同步（WebRTC）等（见设计文档「扩展方向」）。
6. **仓库规范**：补充根目录 `LICENSE` 文件（协议见下文）。
...
| 模式 | 操作 |
|------|------|
| 鸟瞰（默认） | 拖拽旋转、滚轮缩放；鼠标指向地面落点施法 |
| 第一人称 | 点击 HUD「第一人称」切换；点击画面锁定指针，WASD 移动，屏幕中心准星落点施法 |
| 施法 | 输入咒语后点击「施法」或按 **Enter** |

**示例咒语**：`火球术`、`烈焰风暴`、`冰霜之雨`、`感电风暴`、`熔岩喷发`、`雷火旋风`（三元素）
...
└── spell-caster/             # 可运行应用
    ...
    └── src/
        ...
        ├── effects/
        ├── ui/
        └── vfx/              # builders, reactions, VfxLibrary
```
...
> 说明：仓库根目录尚未包含独立的 `LICENSE` 文本文件；对外发布前建议将 MIT 全文放入 `LICENSE` 并与 README 保持一致。
...
- 元素反应与西幻施法玩法的设计灵感，借鉴自常见 RPG 思路（仅作演示，非商业产品）。
...
## 相关文档
- 应用包说明：[`spell-caster/package.json`](spell-caster/package.json)